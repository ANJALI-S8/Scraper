...
def crawl_url_task(
    ...
    try:
        ...
        # Apply article-only check if no-keyword config run is active
        is_keyword_free = not keyword or not keyword.strip() or keyword == "__config__"
        is_no_keyword_config = (source_type in ("direct", "config") and is_keyword_free)
        if is_no_keyword_config:
            if not is_news_article_url(url):
                return url_id, {
                    "status": "skipped",
...
def run_direct_discovery(unique_raw_urls: List[str], query) -> Tuple[Dict[str, List[str]], Set[str]]:
    ...
    def _discover_single(d_url: str) -> Tuple[str, List[str], Optional[str]]:
        ...
        # Regular URL -> link expansion (2 levels: seed page, then a handful of its
        # section/category pages, so we actually reach individual article URLs
        # instead of stopping at the homepage's nav links)
        else:
            candidates = [d_url]
            ...
            def _extract_links(html_text: str, base_for_join: str) -> List[str]:
                found = []
                soup_local = BeautifulSoup(html_text, "html.parser")
                for a in soup_local.find_all("a"):
                    if is_job_stopped(query.id):
                        break
                    href = a.get("href")
                    if not href:
                        continue
                    abs_url = urljoin(base_for_join, href.strip())
                    parsed_abs = urlparse(abs_url)
                    ...
                    # Only relax the domain match for URLs the user typed in
                    # themselves ("direct"). Config-file runs ("config", resolved
                    # from config/urls.json) keep the original strict exact-domain
                    # match on purpose — the whole point of listing specific URLs
                    # in the config file is to control exactly which sites get
                    # crawled, so we shouldn't silently pull in sibling subdomains
                    # there.
                    is_manual_direct = getattr(query, "source_type", "direct") == "direct"
                    if is_manual_direct:
                        # Same-site check, subdomain-aware — mirrors is_chinese_url()'s
                        # approach in url_classifier.py. Strict "abs_domain == domain"
                        # would drop legitimate same-site editions like
                        # global.chinadaily.com.cn when seeded from www.chinadaily.com.cn.
                        is_same_site = (
                            abs_domain == domain
                            or abs_domain.endswith("." + domain)
                            or domain.endswith("." + abs_domain)
                        )
                    else:
                        is_same_site = (abs_domain == domain)

                    if not is_same_site:
                        continue
                    abs_path = parsed_abs.path.lower()
                    if any(abs_path.endswith(ext) for ext in [".pdf", ".jpg", ".jpeg", ".png", ".gif", ".svg", ".zip", ".css", ".js", ".xml"]):
                        continue
            ...
            try:
                ...
                # --- Level 1: fetch the seed page itself ---
                html_text = local_crawler.fetch_page(d_url, engine="fast", ignore_robots=getattr(query, 'ignore_robots', False))
                if is_job_stopped(query.id):
                    return d_url, [d_url], domain

                level1_links = _extract_links(html_text, d_url)
                count = 0
                for link in level1_links:
                    ...
                    count += 1
                    if is_news_article_url(link):
                        article_like_count += 1
                    else:
                        # Likely a homepage nav / section / category page — a
                        # candidate to expand one level deeper below
                        section_links.append(link)

                print(f"[LinkExpansion] Level 1: {len(candidates)} URLs "
                      f"({article_like_count} article-like) for: {d_url}")

                # --- Level 2: only bother if the seed page itself didn't already
                # surface many real article links (e.g. it was just a homepage
                # with a nav bar). Follow a small, capped number of section
                # pages one level further to pull out their article links. ---
                if article_like_count < 8 and section_links and not is_job_stopped(query.id):
                    # De-dupe + cap how many section pages we bother crawling,
                    # to keep this bounded (shared 45s watchdog covers all seeds).
                    seen_sections = list(dict.fromkeys(section_links))[:6]
                    for section_url in seen_sections:
                        if is_job_stopped(query.id) or len(candidates) >= 300:
                            break
                        try:
                            section_html = local_crawler.fetch_page(
                                section_url, engine="fast",
                                ignore_robots=getattr(query, 'ignore_robots', False)
                            )
                        except Exception as ex:
                            print(f"[LinkExpansion] Level 2 fetch failed for {section_url}: {ex}")
                            continue
                        if is_job_stopped(query.id):
                            break
                        for link in _extract_links(section_html, section_url):
                            if len(candidates) >= 300:
                                break
                            if link in candidates:
                                continue
                            # Prioritize genuine article-shaped URLs at this depth;
                            # skip further nav/category links to avoid link-farming.
                            if is_news_article_url(link):
                                candidates.append(link)

                    print(f"[LinkExpansion] Level 2 expansion complete: {len(candidates)} total URLs for: {d_url}")
            except Exception as ex:
                print(f"[WARNING] Failed to expand links for {d_url}: {ex}")
            finally:
                local_crawler.close()
...
def _process_single_keyword(
    ...
    try:
        ...
        if crawl_only and discovered_urls is not None:
            candidate_urls = list(dict.fromkeys(discovered_urls))
        elif query.source_type in ("direct", "config"):
            searched_domains = set()
            if pre_discovered_candidates is not None:
                raw_direct_urls = list(pre_discovered_candidates.keys())
                searched_domains = set(pre_discovered_domains or [])
...
def process_search_query(search_id: int):
    ...
    try:
        ...
        if query.source_type in ("direct", "config"):
            raw_direct_urls = [line.strip() for line in (query.direct_urls or "").split("\n") if line.strip()]
            unique_raw_urls = list(dict.fromkeys(raw_direct_urls))
            print(f"[Direct Discovery] Running keyword-independent discovery for {len(unique_raw_urls)} unique URLs "
                  f"(source_type={query.source_type})...")
            pre_discovered_candidates, pre_discovered_domains = run_direct_discovery(unique_raw_urls, query)
        elif query.source_type == "sitemap":
            from backend.sitemap_discovery import discover_from_sitemap
            base_url = (query.direct_urls or "").strip().splitlines()[0].strip()
